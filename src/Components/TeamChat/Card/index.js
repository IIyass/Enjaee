...
const TeamChatCard = (props) => {
  const {
    next, NextCode, MyTeamChat, ConfirmationModel, step, id, me, name, detail = "CEO Hitachy",
  } = props;
  const [teamId, setTeamId] = useState('')
  const [userName1] = useUserName(id)
  const [myName] = useUserName(me.id)
  const [receiveName] = useUserName(teamId)
  ...
  useEffect(() => {
    setTeamId(MyTeamChat[0].id)
    setCode(MyTeamChat[0].code)
    ConfirmationModel()

  }, [ConfirmationModel, MyTeamChat, me.id])

  console.log(MyTeamChat)
  const Card = () => {
    switch (step) {
      case 1:
        return (
